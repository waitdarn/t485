var clicky_obj = clicky_obj || (function() {
    ...
    function _ins() {
        ...
        this.pageview = function(only_once, extra) {
            ...
            if (_self.ref && _self.ref.match(/^https?:\/\/(www\.)?semalt.com/)) return;
        ...
        this.queue_process = function() {
            var q = _self.queue_get();
            try {
                if (q.heatmap.length || q.events.length) _self.queue_reset();
                for (var i = 0, s = ''; i < q.heatmap.length; i++) {
                    var o = q.heatmap[i];
                    if (!o.href || !o.x || !o.y || !o.w) return;
                    s += '&heatmap[]=' + _self.enc(o.href) + '|' + o.x + '|' + o.y + '|' + o.w;
                }
                if (s) _self.beacon('heatmap', s);
        ...
        this.goal = function(id, revenue, noq) {
            ...
            if (noq || !_self.queue_add(goal)) _self.beacon('goal', goal);
        };
...
try{clicky.init(100869133)}catch(e){}